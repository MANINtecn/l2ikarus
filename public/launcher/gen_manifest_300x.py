"""
Gera o manifest-300x.json (atualizacao incremental do cliente 300x).
...
COMO FUNCIONA
  ...
  vindo do cliente 300x .rar (bloco "drivePackages", preservado deste arquivo).

FLUXO DE USO
  1) Subir a pasta do cliente pro Drive (uma vez; depois so o que mudou):
       rclone sync "C:\\TECX SOFTHOUSE\\L2 IKARUS INTERCROW\\L2 INTERLUDE ACIS\\Cliente l2interlud\\Interlud" gdrive:l2ikarus/300x --progress

  2) Listar os fileIds:
       rclone lsjson gdrive:l2ikarus/300x -R --files-only > drive_listing_300x.json

  3) Gerar o manifest:
       python gen_manifest_interlude.py

  4) Publicar o manifest-300x.json no site.

REGRA DE OURO
  O manifest tem que sair do MESMO cliente que virou o cliente 300x .rar. Se o RAR
...
# ---------------------------------------------------------------------------
CLIENT_DIR = r"C:\TECX SOFTHOUSE\L2 IKARUS INTERCROW\L2 IKARUS PVP 300x\Cliente Interlud 300x"

# Prefixo dos caminhos no manifest. DIFERENTE DO INTERLUDE: o cliente do 300x tem
# o "system\" direto na raiz (nao ha pasta interna), e o launcher instala em
# "L2 Ikarus PVP 300x" rodando "system\l2.exe" — entao os caminhos vao SEM prefixo.
PATH_PREFIX = ""

# Saida do "rclone lsjson ... -R --files-only"
DRIVE_LISTING = "drive_listing_300x.json"

# Manifest existente (de onde herdamos o bloco drivePackages).
MANIFEST_OUT = "manifest-300x.json"
...
SKIP_FILES = {
    ...
    # ---- l2.ini: LIBERADO na PRIMEIRA publicacao do 300x (2026-09-01) ----
    # O jogador precisa receber ServerAddr=192.99.110.164 / Port=2107 uma vez.
    # DEPOIS DA PRIMEIRA PUBLICACAO, DESCOMENTAR A LINHA ABAIXO pelos mesmos motivos
    # do Interlude: sobrescreve resolucao/graficos do jogador a cada update, e um
    # teste local com 127.0.0.1 derruba o login de todo mundo.
    # "l2.ini",
}
...
if not os.path.exists(DRIVE_LISTING):
    raise SystemExit(
        ...
        "Rode antes:\n"
        "  rclone lsjson gdrive:l2ikarus/300x -R --files-only > drive_listing.json"
    )
...
# --- relatorio --------------------------------------------------------------
print("\n" + "=" * 70)
print(f"manifest-300x.json gerado — versao {manifest['version']}")
...
if grandes:
    ...
    print("  -> esses so chegam no jogador pelo cliente 300x .rar da instalacao.")
